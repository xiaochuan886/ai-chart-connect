...
> **配置面板是数据驱动的**：工作台的属性面板**完全由 `config.schema.json`
> 自动生成**——你不需要写任何面板 UI。每个 `property` 变成一个控件，
> 控件类型由 `type` 决定：`boolean`→开关、`select`→下拉(需带 `options`)、
> `color`→取色器、`number`→数字框、`string`/`multiline`→文本框。`group`
> 决定它归到哪个折叠分区(中文标题由英文 token 自动翻译)。
>
> 所以"显示/隐藏 X"这种开关**必须**用 `"type": "boolean"`，不要用 string 建模。
> 开关的推荐归属：`showTitle`→`text`、`showLegend`→`legend`、`showLabels`→`labels`、
> 图表专属开关(如 `showStates`)→`chart`。不要发明新的 group 值(只认上面 9 个)。
>
> **`show*` 开关由 layout shell 处理，不是 renderer**：只要 renderer 挂了对应的
> slot 组件(`<Title>`/`<Subtitle>`/`<Legend>`/`<Footnote>`)且 schema 声明了对应
> boolean，shell 会读 `config.showTitle`/`showSubtitle`/`showLegend`/`showFootnote`
> 自动 dock/omit 该区域——renderer 不需要自己 `if (showTitle)` 判断。
> (注意键名是 **`showFootnote`**，与 `<Footnote>` 组件名一致，不是 `showFooter`。)
